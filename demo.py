...
if __name__ == '__main__': 
    ...
    with open(join('./config/', opt.id + '.yaml')) as f:
        # Add safe_load and error handling
        try:
            config = yaml.safe_load(f)
            if not config:
                raise ValueError(f"Config file for ID '{opt.id}' is empty")
    ...
    Featopt.map_location = 'cpu' if opt.device == 'cpu' else None
    Audio2Feature = create_model(Featopt)   
    Audio2Feature.setup(Featopt)  
    Audio2Feature.eval()     
    print('---------- Loading Model: {} -------------'.format(Headopt.task))
    Headopt.map_location = 'cpu' if opt.device == 'cpu' else None
    ...
    Renderopt.map_location = 'cpu' if opt.device == 'cpu' else None
